...
const Editproduct = () => {
  ...
  useEffect(() => {
    ...
    setProductsubCategory(location?.state?.subcategory)
    setproductweight(location?.state?.productweight)
    setproductSize(location?.state?.productSize)
    setProductThickness(location?.state?.productThickness)
  }, [location?.state]);
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const obj = {
        ...
        returnPolicy,
        countryOfOrigin,
        productSize,productThickness,productweight
      };
      console.log(obj);

      const response = await axios.put(
        `${apiurl}/product/update-product?id=${location?.state?._id}`,obj,
        {
          headers: {
